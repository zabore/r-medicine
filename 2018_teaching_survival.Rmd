...
ggsave("./img/surv_curve_sq.png", surv_curve_sq$plot, width = 3.5, 
       height = 3)
```

\begin{columns}
  \begin{column}{0.6\linewidth}
    \includegraphics[scale = 0.75]{surv_curve_sq}
  \end{column}
  
  \begin{column}{0.4\linewidth}
  \begin{itemize}
  \setlength\itemsep{1em}
    \item[] In the lung data this would lead to an estimate of survival probability at the end of the study of $1 - \frac{165}{228} = 0.28$.
...
ggsave("./img/wrong_pct.png", wrong_pct$plot, width = 3.5, height = 3.5)
```

\begin{columns}
  \begin{column}{0.6\linewidth}
    \includegraphics[scale = 0.75]{wrong_pct}
  \end{column}
...
\begin{columns}
  ...
  \setlength\itemsep{1em}
    ...
    \item[] But the \textcolor{blue}{blue} curve is clearly lower than the \textcolor{orange}{orange} curve, especially at the end 
...
\begin{columns}
  ...
  \begin{column}{0.4\linewidth}
  This occurs because the censored subjects only contribute information for \textbf{part} of the follow-up time, and then they fall out of the denominator, thus pulling down the cumulative probability of survival.
  \end{column}
\end{columns}
...
\begin{columns}
  ...
  \begin{itemize}
  \setlength\itemsep{1em}
    \item[] In the lung data the 1-year survival probability is `r round(est_1_year, 2)`
...
ggsave("./img/x_year_est_1.png", x_year_est_1$plot, width = 3.5, height = 3)
```

\begin{columns}
  \begin{column}{0.6\linewidth}
    \includegraphics[scale = 0.75]{x_year_est_1}
  \end{column}
  
  \begin{column}{0.4\linewidth}
  The 1-year survival probability is the \textbf{probability on the y-axis} corresponding to \textbf{1-year on the x-axis}
  \end{column}
\end{columns}
...
ggsave("./img/x_year_est_2.png", x_year_est_2$plot, width = 3.5, height = 3)
```

\begin{columns}
  \begin{column}{0.6\linewidth}
    \includegraphics[scale = 0.75]{x_year_est_2}
  \end{column}
...
```{r}
ggsave("./img/x_year_est_3.png", x_year_est_2$plot, width = 5, height = 3.5)
```

\begin{center}
    \includegraphics[scale = 0.6]{x_year_est_3}
\end{center}
...
\begin{columns}
  ...
  \begin{itemize}
  \setlength\itemsep{1em}
    \item[] In the lung data the median survival time is `r round(est_median, 1)` days
    \item[] But where does this number come from?
    \item[] And what does it mean?
  \end{itemize}
  \end{column}
...
ggsave("./img/median_est_1.png", median_est_1$plot, width = 3.5, height = 3)
```

\begin{columns}
  \begin{column}{0.6\linewidth}
    \includegraphics[scale = 0.75]{median_est_1}
  \end{column}
...
ggsave("./img/median_est_2.png", median_est_2$plot, width = 3.5, height = 3)
```

\begin{columns}
  \begin{column}{0.6\linewidth}
    \includegraphics[scale = 0.75]{median_est_2}
  \end{column}
...
The median survival time is `r round(res["median"])` days (95% CI: `r round(res["0.95LCL"])` - `r round(res["0.95UCL"])`), which represents the estiamted point in time that half of subjects will live beyond.


## Finally put everything together into a report to our collaborator about the median survival time

```{r}
ggsave("./img/median_est_3.png", median_est_2$plot, width = 5, height = 3.5)
```

\begin{center}
    \includegraphics[scale = 0.6]{median_est_3}
\end{center}
...
\begin{columns}
  ...
  \begin{itemize}
  \setlength\itemsep{1em}
    \item[] In the lung data this would lead to an estimate of median survival time of `r summary(lung$time[lung$status == 2])["Median"]` days
    \item[] But this is \textbf{incorrect}. 
    \item[] Why?
  \end{itemize}
  \end{column}
...
ggsave("./img/wrong_median_1.png", wrong_median_1$plot, width = 3.5, 
       height = 3.5)
```

\begin{columns}
  \begin{column}{0.6\linewidth}
    \includegraphics[scale = 0.75]{wrong_median_1}
  \end{column}
...
ggsave("./img/wrong_median_2.png", wrong_median_2$plot, width = 3.5, 
       height = 3.5)
```

\begin{columns}
  \begin{column}{0.6\linewidth}
    \includegraphics[scale = 0.75]{wrong_median_2}
  \end{column}
...
\begin{itemize}
  \setlength\itemsep{1em}
    ...
    \item The median survival time is clearly longer, at $310$ days.
  \end{itemize}
  \end{column}
\end{columns}
...
ggsave("./img/median_nr_1.png", median_nr_1$plot, width = 3.5, height = 3)
```

\begin{columns}
  \begin{column}{0.6\linewidth}
    \includegraphics[scale = 0.75]{median_nr_1}
  \end{column}
  
  \begin{column}{0.4\linewidth}
\begin{itemize}
  \setlength\itemsep{1em}
    \item Survival curve based on a sample of lung data subjects
...
ggsave("./img/median_nr_2.png", median_nr_2$plot, width = 3.5, height = 3)
```

\begin{columns}
  \begin{column}{0.6\linewidth}
    \includegraphics[scale = 0.75]{median_nr_2}
  \end{column}
