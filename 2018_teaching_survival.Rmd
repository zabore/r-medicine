...
\includegraphics[scale = 0.5]{pubs}
...
\begin{itemize}
  ...
  \item[\checkbox] Accompany numbers such as p-values and hazard ratios with detailed \textbf{interpretations}
\end{itemize}
...
\begin{columns}
  ...
  \setlength\itemsep{1em}
    ...
    \item[] The \textcolor{blue}{blue} curve is clearly lower than the \textcolor{orange}{orange} curve, especially at the end.
...
ggsave("./img/wrong_median_3.png", wrong_median_3$plot, width = 3.5, 
       height = 3.5)
```

\begin{columns}
  \begin{column}{0.6\linewidth}
    \includegraphics[scale = 0.75]{wrong_median_3}
  \end{column}
...
ggsave("./img/haz_plot.png", haz_plot$plot, width = 3.5, height = 3.5)
```

\begin{columns}
  \begin{column}{0.6\linewidth}
    \includegraphics[scale = 0.75]{haz_plot}
  \end{column}
  
  \begin{column}{0.4\linewidth}
\begin{itemize}
  \setlength\itemsep{1em}
    \item The hazard ratio (HR) represents the difference in hazards between two groups at any particular point in time
    \item HR = `r round(exp(broom::tidy(cph4)["estimate"]), 2)` comparing male to female with respect to hazard of death in the \texttt{lung} data
  \end{itemize}
  \end{column}
\end{columns}
...
haz_plot_2$plot <- haz_plot_2$plot + 
  annotate("text", x = 0, y = 3, label = glue::glue(
    "The hazard ratio for males\nversus females with respect\nto hazard of death ", 
    "is {round(exp(broom::tidy(cph4)['estimate']), 2)}."), 
    col = msk_palette("contrast")[12], size = 4, hjust = 0)
  
ggsave("./img/haz_plot_2.png", haz_plot_2$plot, width = 3.5, height = 3.5)
```

\begin{columns}
  \begin{column}{0.6\linewidth}
    \includegraphics[scale = 0.75]{haz_plot_2}
  \end{column}
...
\begin{itemize}
  \setlength\itemsep{1em}
    ...
    \item So our HR = `r round(exp(broom::tidy(cph4)['estimate']), 2)` implies that around 0.4 times as many females are dying as males, at any given time
  \end{itemize}
  \end{column}
\end{columns}
